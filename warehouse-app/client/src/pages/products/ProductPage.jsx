...
export default function ProductPage() {
  ...
  const isNew = id === 'new'
  const [form, setForm] = useState({ product_name: '', type_id: '', unit_id: '', price: '', has_bom: false })
  ...
  const addLine = () => setLines([...lines, { material_code: '', quantity_needed: '' }])
  ...
  const save = async () => {
    try {
      const payload = { ...form, price: parseFloat(form.price),
        line_items: form.has_bom ? lines.map(l => ({ material_code: l.material_code, quantity_needed: parseFloat(l.quantity_needed) })) : [] }
        ...
        {form.has_bom && (
          <div style={{ marginTop: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <strong style={{ fontSize: 13 }}>Bill of Materials</strong>
              <button className="btn-primary" onClick={addLine}>+ Add Line</button>
            </div>
            <table>
              <thead>
                <tr><th>No</th><th>Material Code</th><th>Material Name</th><th>Qty Needed</th><th>Unit</th><th>Unit Price</th><th>Total Value</th><th></th></tr>
              </thead>
              <tbody>
                {lines.map((l, i) => {
                  const mat = allProducts.find(p => p.product_code === l.material_code)
                  ...
                  return (
                    <tr key={i}>
                      <td>{i + 1}</td>
                      <td>
                        <select value={l.material_code} onChange={e => updateLine(i, 'material_code', e.target.value)} style={{ width: 110 }}>
                          <option value="">— Select —</option>
                          {allProducts.map(p => <option key={p.product_code} value={p.product_code}>{p.product_code}</option>)}
                        </select>
                      </td>
                      <td className="text-muted">{mat?.product_name || '—'}</td>
                      <td><input type="number" value={l.quantity_needed} onChange={e => updateLine(i, 'quantity_needed', e.target.value)} style={{ width: 80 }} min="0" step="0.001" /></td>
                      <td className="text-muted">{mat?.unit_name || '—'}</td>
                      <td className="text-muted">{up.toFixed(2)}</td>
                      <td className="text-muted">{(up * qty).toFixed(2)}</td>
                      <td><button className="btn-danger" onClick={() => removeLine(i)}>Del</button></td>
                    </tr>
                  )
                })}
                {!lines.length && <tr><td colSpan={8} className="text-center text-muted">No lines — click + Add Line</td></tr>}
                {lines.length > 0 && (
                  <tr className="row-total">
                    <td colSpan={6} className="text-right">TOTAL BOM COST</td>
                    <td>{totalBom.toFixed(2)}</td><td></td>
                  </tr>
                )}
              </tbody>
            </table>
